...
if ($_FILES['csv_file']['error'] === UPLOAD_ERR_OK) {
    ...
    if (validateCsv($csvFile)) {
        // 删除原表
        $db->exec('DROP TABLE IF EXISTS conference_info');
        ...
        if (($handle = fopen($csvFile, "r"))!== FALSE) {
            ...
            // 创建表并添加自增 ID 列
            $createTableQuery = "CREATE TABLE conference_info (id INTEGER PRIMARY KEY AUTOINCREMENT";
            ...
            while (($data = fgetcsv($handle))!== FALSE) {
                ...
                $query = "INSERT INTO conference_info (".implode(',', $columnNames).") VALUES (".implode(',', $values).")";
                $db->exec($query);
                $rowNumber++;
            }
            fclose($handle);
