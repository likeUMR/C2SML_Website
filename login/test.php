...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
    <div class="max-w-md w-full space-y-8 bg-white p-10 rounded-lg shadow-lg">
        <div class="text-center">
            <!-- <img src="/placeholder.svg" alt="Logo" class="mx-auto h-12 w-auto"> -->
        ...
        <form class="mt-8 space-y-6" action="" method="POST">
            ...
            <div class="flex items-center justify-between">
                <div class="text-sm">
                    <a href="reset_password.php" class="font-medium text-blue-600 hover:text-blue-500">忘记密码？</a>
                </div>
            </div>
        ...
        <div class="mt-4 text-center">
            <a href="/index.php" class="text-blue-500 hover:text-blue-700">返回主页</a>
        </div>
    </div>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            ...
            // echo "<script>alert('跳转到的URL: " . addslashes($redirect_url) . "');</script>";
            header("Location: " . urldecode($redirect_url));
            exit();
        } else {
            echo "<script>alert('用户名或密码无效，请重试。');</script>";
        }
    }
    ?>
</body>
